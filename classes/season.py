...
class SolarYear:
	...
	def calc_season(self):
		...
		season_offset = self._year.cyclesToYear * SOLAR_CYCLE_EXCESS - SEASON_BEFOR_FIRST_MOLAD
		# Add the offset of each year
		season_offset += (self._year.placeInCycle -1) * SOLAR_YEAR_EXCESS
		...
		if self.season_time.parts != 0:
			...
			if self._year.placeInCycle in LEAP_YEARS:
				thisMonth = Month(self._year, 13)
	...
	def season_time_of_week(self, seasonNumber=0):
		...
		place_in_long_cycle = (self._year.yearsFromCreation -1) % LONG_CYCLE_LENGTH
