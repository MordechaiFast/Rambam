...
def test_month_lengths():
    # 8:5
    for month in Year(1):
        if   month.month_count in {1, 5, 7, 9, 11}:  #Heshvan, (Teves), Addar, Iyyar, Tamuz, Ellul
            assert month.two_day_Rosh_Chodesh
        elif month.month_count in {0, 4, 6, 8, 10}:     #Tishrei, (Kislev), Shevat, Nissan, Sivan, Av
            assert not month.two_day_Rosh_Chodesh

    for month in Year(3):
        if   month.month_count in {1, 5, 6, 8, 10, 12}:
            assert month.two_day_Rosh_Chodesh
        elif month.month_count in {0, 4, 7, 9, 11}:
            assert not month.two_day_Rosh_Chodesh
def test_variable_months():
    ...
    for _ in range(1, 100):
        thisYear = nextYear
        nextYear = thisYear.year_after
        daysBetween = (nextYear.Rosh_Hashana - thisYear.Rosh_Hashana - 1) % 7
        if thisYear.place_in_cycle not in LEAP_YEARS:
            if   daysBetween == 2:
